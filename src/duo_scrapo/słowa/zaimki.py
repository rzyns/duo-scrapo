...
from duo_scrapo.słowa import GenderedSingular, Numbered, SupportsAnkiExport
...
@define
class FormyZaimków(Numbered[GenderedSingular[FormyPrzypadków[str]], GenderedSingular[FormyPrzypadków[str]]], SupportsAnkiExport):
    @classmethod
    def get_cols(cls):
    ...
    @classmethod
    def empty(cls, empty_lp: Callable[[], GenderedSingular[FormyPrzypadków[str]]], empty_lm: Callable[[], GenderedSingular[FormyPrzypadków[str]]]):
        return cls(
...
model = Model(
    ...
    fields=[
        {"name": col}
        for col in ("en", "pl", *FormyZaimków.get_cols())
    ],
    templates=[
        asdict(a)
        for a in templates
